...
{{tenantName}}`,
      ...
      expect(result.instructions).toContain(
        expect.stringContaining('Widerspruch')
      );
    });
    ...
    it('should format dates correctly', async () => {
      const mockTemplate = {
        ...
        category: 'REPAIRS' as LegalCategory,
        language: 'de',
        isActive: true,
        createdAt: new Date(),
        updatedAt: new Date()
    ...
    it('should throw error for missing required fields', async () => {
      ...
      await expect(
        service.generateDocument('template-4', incompleteData, 'user-1')
      ).rejects.toThrow('Template-Validierung fehlgeschlagen');
    });
    ...
    it('should throw error for inactive template', async () => {
      ...
      await expect(
        service.generateDocument('template-5', {}, 'user-1')
      ).rejects.toThrow('Template ist nicht aktiv');
    });
  });
  ...
  describe('listTemplates', () => {
    ...
    it('should filter templates by category', async () => {
      ...
      const result = await service.listTemplates('RENT_REDUCTION' as LegalCategory);
  ...
  describe('generateInstructions', () => {
    it('should generate specific instructions for rent reduction', async () => {
      ...
      expect(result.instructions).toContain(
        expect.stringContaining('Einschreiben')
      );
      expect(result.instructions).toContain(
  ...
  describe('generateLegalNotes', () => {
    it('should generate category-specific legal notes', async () => {
      ...
      expect(result.legalNotes).toContain(
        expect.stringContaining('§ 536 BGB')
      );
      expect(result.legalNotes).toContain(
        expect.stringContaining('Rechtsberatung')
      );
    });
  });
});
