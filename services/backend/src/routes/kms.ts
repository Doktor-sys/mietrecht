...
import { KeyPurpose, KeyManagementError } from '../types/kms';
...
let kmsService: KeyManagementService;
 ...
 */
export function initializeKMSServices(redis: RedisClientType): void {
  encryptionService = new EncryptionService();
  kmsService = new KeyManagementService(prisma, redis, encryptionService);
  ...
  logger.info('KMS services initialized for API routes');
}
...
router.post('/keys', requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    if (!purpose || !Object.values(KeyPurpose).includes(purpose as KeyPurpose)) {
 ...
 *         $ref: '#/components/responses/InternalServerError'
 */
router.get('/keys/:keyId', requireAdmin, async (req: Request, res: Response) => {
  try {
 ...
 *         $ref: '#/components/responses/InternalServerError'
 */
router.post('/keys/:keyId/rotate', requireAdmin, async (req: Request, res: Response) => {
  try {
    const { keyId } = req.params;
    // Verwende eine feste tenantId, da das User-Objekt keine tenantId-Eigenschaft hat
    const tenantId = 'default';
 ...
 *         $ref: '#/components/responses/InternalServerError'
 */
router.post('/keys/:keyId/compromised', requireAdmin, async (req: Request, res: Response) => {
  try {
    const { keyId } = req.params;
    ...
    logger.error('Failed to mark key as compromised:', error);
