...
describe('GDPRComplianceService', () => {
  ...
  beforeEach(() => {
    ...
    gdprService = new GDPRComplianceService(mockEncryptionService);
  });
  ...
  describe('exportUserData', () => {
    ...
    it('sollte Fehler werfen wenn Nutzer nicht existiert', async () => {
      ...
      await expect(
        gdprService.exportUserData('non-existent')
      ).rejects.toThrow('Nutzer nicht gefunden');
    });
  });
  ...
  describe('updateConsent', () => {
    it('sollte Einwilligungen granular verwalten', async () => {
      ...
      (prisma.userConsent.upsert as jest.Mock).mockResolvedValue({
        userId,
        ...consents,
        updatedAt: new Date(),
      });
  ...
  describe('getConsentStatus', () => {
    it('sollte aktuellen Einwilligungsstatus abrufen', async () => {
      ...
      (prisma.userConsent.findUnique as jest.Mock).mockResolvedValue(mockConsent);
  ...
  describe('generateDataPortabilityExport', () => {
    it('sollte Daten in maschinenlesbarem Format exportieren', async () => {
      ...
      (prisma.conversation.findMany as jest.Mock).mockResolvedValue(mockData.conversations);
      (prisma.document.findMany as jest.Mock).mockResolvedValue(mockData.documents);
