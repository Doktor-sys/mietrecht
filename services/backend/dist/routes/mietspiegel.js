...
const MietspiegelController_1 = require("../controllers/MietspiegelController");
const auth_1 = require("../middleware/auth");
const validation_1 = require("../middleware/validation");
const express_validator_1 = require("express-validator");
const router = (0, express_1.Router)();
const prisma = new client_1.PrismaClient();
const mietspiegelController = new MietspiegelController_1.MietspiegelController(prisma);
/**
 * @swagger
 * components:
 *   schemas:
 ...
 */
router.get('/:city', auth_1.authenticateToken, [
 ...
 *         description: Nicht authentifiziert
 */
router.post('/calculate-rent', auth_1.authenticateToken, [
    (0, express_validator_1.body)('city')
        .notEmpty()
        .withMessage('Stadt ist erforderlich')
        .isLength({ min: 2, max: 50 })
        .withMessage('Stadt muss zwischen 2 und 50 Zeichen lang sein'),
    (0, express_validator_1.body)('apartmentDetails')
        .notEmpty()
        .withMessage('Wohnungsdetails sind erforderlich'),
    (0, express_validator_1.body)('apartmentDetails.size')
        .isFloat({ min: 1 })
        .withMessage('Wohnungsgröße muss größer als 0 sein'),
    (0, express_validator_1.body)('apartmentDetails.rooms')
        .isFloat({ min: 1 })
        .withMessage('Anzahl Zimmer muss größer als 0 sein'),
    (0, express_validator_1.body)('apartmentDetails.constructionYear')
        .optional()
        .isInt({ min: 1800, max: new Date().getFullYear() })
        .withMessage('Baujahr muss zwischen 1800 und heute liegen'),
    (0, express_validator_1.body)('apartmentDetails.condition')
        .optional()
        .isIn(['simple', 'normal', 'good', 'excellent'])
        .withMessage('Zustand muss simple, normal, good oder excellent sein'),
    (0, express_validator_1.body)('apartmentDetails.location')
        .optional()
 ...
 *         description: Nicht authentifiziert
 */
router.get('/:city/regulations', auth_1.authenticateToken, [
    (0, express_validator_1.param)('city')
        .notEmpty()
        .withMessage('Stadt ist erforderlich')
        .isLength({ min: 2, max: 50 })
 ...
 *         description: Nicht authentifiziert
 */
router.post('/compare-rent', auth_1.authenticateToken, [
    (0, express_validator_1.body)('city')
        .notEmpty()
        .withMessage('Stadt ist erforderlich')
        .isLength({ min: 2, max: 50 })
        .withMessage('Stadt muss zwischen 2 und 50 Zeichen lang sein'),
    (0, express_validator_1.body)('currentRent')
        .isFloat({ min: 0.01 })
 ...
 *         description: Nicht authentifiziert
 */
router.get('/cities', auth_1.authenticateToken, async (req, res, next) => {
    try {
 ...
 */
router.put('/update', auth_1.authenticateToken, auth_1.requireAdmin, [
    (0, express_validator_1.body)('city')
        .notEmpty()
        .withMessage('Stadt ist erforderlich')
        .isLength({ min: 2, max: 50 })
    ...
    (0, express_validator_1.body)('year')
        .isInt({ min: 2000, max: new Date().getFullYear() + 1 })
        .withMessage('Jahr muss zwischen 2000 und nächstem Jahr liegen'),
    (0, express_validator_1.body)('averageRent')
        .isFloat({ min: 0.01 })
        .withMessage('Durchschnittsmiete muss größer als 0 sein'),
    (0, express_validator_1.body)('rentRanges')
        .isArray({ min: 1 })
        .withMessage('Mindestens eine Mietspanne ist erforderlich'),
    (0, express_validator_1.body)('specialRegulations')
        .optional()
        .isArray()
