...
describe('MietspiegelService Unit Tests', () => {
    ...
    beforeEach(() => {
        ...
        mietspiegelService = new MietspiegelService_1.MietspiegelService(prismaMock);
    });
    ...
    describe('calculateRentRange', () => {
        ...
        it('sollte Fehler werfen bei ungültigen Wohnungsdetails', async () => {
            ...
            await expect(mietspiegelService.calculateRentRange('Berlin', invalidDetails)).rejects.toThrow('Wohnungsgröße muss größer als 0 sein');
        });
        it('sollte Fehler werfen, wenn keine Mietspiegel-Daten verfügbar sind', async () => {
            prismaMock.mietspiegelData.findUnique.mockResolvedValue(null);
            prismaMock.mietspiegelData.findFirst.mockResolvedValue(null);
            await expect(mietspiegelService.calculateRentRange('UnbekannteStadt', validApartmentDetails)).rejects.toThrow('Keine Mietspiegel-Daten für UnbekannteStadt verfügbar');
        });
    });
    ...
    describe('compareMietWithMietspiegel', () => {
        ...
        it('sollte Mietvergleich korrekt durchführen', async () => {
            ...
            prismaMock.mietspiegelData.findUnique.mockResolvedValue(mockMietspiegelData);
            const result = await mietspiegelService.compareMietWithMietspiegel('Berlin', 1000, validApartmentDetails);
    ...
    describe('getAvailableCities', () => {
        it('sollte verfügbare Städte mit Informationen zurückgeben', async () => {
            ...
            prismaMock.mietspiegelData.groupBy.mockResolvedValue(mockGroupByResult);
            prismaMock.mietspiegelData.findMany.mockResolvedValue(mockCityData);
    ...
    describe('updateMietspiegelData', () => {
        it('sollte Mietspiegel-Daten erfolgreich aktualisieren', async () => {
            ...
            prismaMock.mietspiegelData.upsert.mockResolvedValue(mockUpdatedData);
        ...
        it('sollte Validierungsfehler für ungültige Daten werfen', async () => {
            ...
            await expect(mietspiegelService.updateMietspiegelData(invalidUpdateData)).rejects.toThrow('Stadt ist erforderlich');
        });
    });
});
