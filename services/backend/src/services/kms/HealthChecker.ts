...
export class HealthChecker {
  ...
  async checkHealth(): Promise<HealthStatus> {
    ...
    try {
      ...
      const allHealthy = 
        masterKeyHealth.status === 'healthy' &&
      ...
      logger.info(`Health check completed in ${duration}ms - Status: ${allHealthy ? 'HEALTHY' : 'UNHEALTHY'}`);
   ...
   */
  private async checkMasterKey(): Promise<ComponentHealth> {
    const startTime = Date.now();
   ...
   */
  private async checkRotation(): Promise<ComponentHealth> {
    const startTime = Date.now();
   ...
   */
  isCheckRecent(maxAgeMs: number = 60000): boolean {
