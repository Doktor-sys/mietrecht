...
- NJW-Datenintegration
	- `scripts/njw_api_client.js` — NJW-API-Client zum Abruf von NJW-Artikeln
...
- Verbesserte Data Sources
	...
	- Retry: Exponentielles Backoff (3 Versuche, Basisverzögerung 1000 ms)
...
- Backend — Enhanced APIs (ML/NLP)
	...
	- Beispiel: Antwort enthält `"isEnhanced": true` und `"confidence": 0.9`
...
- Mobile — Offline & Push
	...
	- Daten-Ablaufzeit: 7 Tage (konfigurierbar)
	- Push-Kanäle: `default`, `chat`, `legal_updates` und `reminders` mit Zeitplanungs-Funktionen
	- E2E-Tests: `e2e/tests/offline.e2e.js` (`npm run e2e:test`)

- Qualität & Visual Regression
	- Web: Playwright-Einstellungen in `web-app/playwright.config.ts`, Toleranzschwelle 0,1 %
	- Mobile: Detox-Einstellungen in `mobile-app/detox.config.js`
	- Lokale Befehle: `npx playwright test` und `npx playwright test --update-snapshots`

- CI/CD & Sicherheit
	- GitHub Actions: automatisierte Visual- und Sicherheits-Tests, Docker-Scanning und Artefakt-Archivierung
	- Sicherheits-Skripte: `scripts/test/verifyEnhancedSecurity.js` und `npm run test:security:enhanced`
	- Zweig-Schutz (Branch-Protection) und aktualisierte Workflow-Definitionen
...
- Visual Regression: strengere Diff-Grenzwerte und CI-Integration zur frühzeitigen Erkennung visueller Regressions
- CI/CD: zusätzliche Sicherheits-Prüfungen sowie erweiterte Lint- und Test-Skripte

### Sonstiges

- Projektweite Dokumentation aktualisiert: Task-Management (Asana-Integration, Migrations-Checklisten), KMS-Verbesserungen und Empfehlungen zu MFA/Zero-Trust
