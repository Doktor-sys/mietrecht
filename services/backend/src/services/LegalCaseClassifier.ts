...
export class LegalCaseClassifier {
   ...
   */
  async classifyCase(query: string): Promise<ClassificationResult> {
    try {
      logger.info('Classifying legal case', { queryLength: query.length });
      ...
      logger.error('Error classifying case', { error });
      throw error;
    }
  }
  ...
  private determineEscalation(
    ...
    context: ContextExtractionResult,
    riskLevel: 'low' | 'medium' | 'high'
  ): { escalationRecommended: boolean; escalationReason?: string } {
  ...
  private determineSubCategory(
    ...
    switch (category) {
      case 'rent_reduction':
        ...
        if (combinedText.includes('schimmel')) return 'mold';
