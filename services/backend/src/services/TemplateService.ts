...
export class TemplateService {
  ...
  async generateDocument(
    ...
    try {
      logger.info('Generating document from template', { templateId, userId });
      ...
      if (!template.isActive) {
        throw new ValidationError('Template ist nicht aktiv');
      }
      ...
      // Log Business Event
      loggers.businessEvent('TEMPLATE_GENERATED', userId, {
        templateId,
   ...
   */
  private fillTemplate(templateContent: string, data: TemplateData): string {
   ...
   */
  private generateInstructions(templateType: string, data: TemplateData): string[] {
    const instructions: string[] = [];

    switch (templateType) {
      case 'rent_reduction':
        instructions.push(
          '1. Prüfen Sie, ob der Mangel tatsächlich vorliegt und dokumentiert ist',
          '2. Setzen Sie dem Vermieter eine angemessene Frist zur Mängelbeseitigung (in der Regel 14 Tage)',
          '3. Versenden Sie das Schreiben per Einschreiben mit Rückschein',
          '4. Bewahren Sie eine Kopie und den Versandnachweis auf',
          '5. Mindern Sie die Miete erst nach Ablauf der Frist, wenn der Mangel nicht behoben wurde'
        );
        
        if (data.rentReductionPercentage && data.rentReductionPercentage > 20) {
          instructions.push(
            'WICHTIG: Bei einer Mietminderung über 20% sollten Sie vorab rechtlichen Rat einholen'
          );
        }
        break;
      ...
      case 'rent_increase_objection':
        instructions.push(
          '1. Prüfen Sie die Begründung der Mieterhöhung',
          '2. Vergleichen Sie mit dem lokalen Mietspiegel',
          '3. Versenden Sie den Widerspruch innerhalb der Frist (meist 2 Monate)',
          '4. Senden Sie das Schreiben per Einschreiben mit Rückschein',
          '5. Zahlen Sie bis zur Klärung weiterhin die alte Miete'
        );
        break;

      case 'deadline_letter':
        instructions.push(
          '1. Beschreiben Sie den Mangel oder das Problem konkret',
          '2. Setzen Sie eine angemessene Frist (mindestens 14 Tage)',
          '3. Versenden Sie per Einschreiben mit Rückschein',
          '4. Dokumentieren Sie den Mangel mit Fotos',
      ...
      case 'utility_bill_objection':
        instructions.push(
          '1. Prüfen Sie die Abrechnung auf Vollständigkeit und Richtigkeit',
          '2. Legen Sie innerhalb von 12 Monaten Widerspruch ein',
          '3. Fordern Sie Einsicht in Belege und Verträge',
          '4. Versenden Sie per Einschreiben mit Rückschein',
          '5. Zahlen Sie nur den unstrittigen Teil'
        );
        break;

      default:
        instructions.push(
          '1. Prüfen Sie alle Angaben auf Richtigkeit',
          '2. Versenden Sie das Schreiben per Einschreiben mit Rückschein',
          '3. Bewahren Sie eine Kopie auf',
          '4. Dokumentieren Sie alle relevanten Umstände'
        );
    }
   ...
   */
  private generateLegalNotes(category: LegalCategory): string[] {
    const notes: string[] = [
    ...
    switch (category) {
      case 'RENT_REDUCTION':
        notes.push(
          'Rechtliche Grundlage: § 536 BGB - Mietminderung bei Mängeln',
          'Die Höhe der Mietminderung richtet sich nach der Schwere des Mangels',
          'Eine zu hohe Mietminderung kann zu Zahlungsrückständen und Kündigung führen'
        );
        break;

      case 'RENT_INCREASE':
        notes.push(
          'Rechtliche Grundlage: § 558 BGB - Mieterhöhung bis zur ortsüblichen Vergleichsmiete',
          'Sie haben 2 Monate Zeit, um der Mieterhöhung zu widersprechen',
          'Bei Widerspruch muss der Vermieter ggf. vor Gericht ziehen'
        );
        break;
      ...
      case 'UTILITY_COSTS':
        notes.push(
          ...
          'Sie haben Anspruch auf Einsicht in alle Belege'
        );
        break;

      case 'TERMINATION':
        notes.push(
          'Rechtliche Grundlage: §§ 573-573c BGB - Kündigung des Mietverhältnisses',
          'Bei Kündigungen sollten Sie unbedingt rechtlichen Rat einholen',
          'Fristen und Formvorschriften müssen streng eingehalten werden'
        );
        break;
      ...
      case 'REPAIRS':
        notes.push(
          ...
          'Der Vermieter ist verpflichtet, die Mietsache in gebrauchsfähigem Zustand zu erhalten',
          'Bei Gefahr im Verzug können Sie Mängel selbst beseitigen und Ersatz verlangen'
        );
        break;

      default:
        notes.push(
          'Bitte informieren Sie sich über die spezifischen rechtlichen Grundlagen Ihres Falls'
        );
    }
   ...
   */
  private validateTemplateData(template: any, data: TemplateData): void {
    const errors: string[] = [];
    ...
    if (errors.length > 0) {
      throw new ValidationError(`Template-Validierung fehlgeschlagen: ${errors.join(', ')}`);
    }
  }
   ...
   */
  async listTemplates(category?: LegalCategory): Promise<TemplateMetadata[]> {
    try {
  ...
  async createTemplate(data: {
    ...
    try {
      ...
      loggers.businessEvent('TEMPLATE_CREATED', '', {
        templateId: template.id,
        name: template.name,
        category: template.category
      });
