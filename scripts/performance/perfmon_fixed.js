...
const { PerformanceReportGenerator } = require('./reportGenerator_fixed.js');
const { PerformanceAlerting } = require('./alerting_fixed.js');
...
async function runMonitor(args) {
  ...
  console.log(`Monitoring started with interval ${options.interval || 60000}ms`);
    ...
    switch (arg) {
      ...
      case '-i':
        ...
        if (!isNaN(interval) && interval > 0) {
          ...
          console.warn('Invalid interval value, using default (60000ms)');
        }
        break;
      case '--format':
      case '-f':
        ...
        if (format === 'json' || format === 'html') {
          ...
          console.warn('Invalid format value, using default (json)');
        }
        break;
      case '--output':
      case '-o':
      ...
      case '--history-limit':
        ...
        if (!isNaN(limit) && limit > 0) {
          ...
          console.warn('Invalid history limit value, using default (10)');
        }
        break;
      case '--config':
      case '-c':
...
function displayReportSummary(report) {
  try {
    ...
    console.error('Error displaying report summary:', error.message);
  }
}
