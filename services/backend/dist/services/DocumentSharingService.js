...
exports.DocumentSharingService = void 0;
...
class DocumentSharingService {
     ...
     */
    async shareDocument(documentId, ownerId, sharedWithEmail, permission = client_1.Permission.READ, expiresAt) {
        try {
            ...
            if (existingShare) {
                throw new errorHandler_1.ValidationError('Document already shared with this user');
            }
            ...
            logger_1.logger.error('Error sharing document', { error, documentId, ownerId });
            throw error;
        }
    }
    /**
    ...
    async getSharedDocuments(userId) {
        try {
            ...
            logger_1.logger.error('Error getting shared documents', { error, userId });
            throw error;
        }
    }
    /**
    ...
    async getDocumentShares(documentId, ownerId) {
        try {
            ...
            logger_1.logger.error('Error getting document shares', { error, documentId, ownerId });
            throw error;
        }
    }
    /**
     ...
     */
    async updateShare(shareId, ownerId, permission, expiresAt) {
        try {
            ...
            if (!share) {
                throw new errorHandler_1.ValidationError('Share not found or access denied');
            }
            // @ts-ignore: TypeScript not recognizing documentSharing property
            ...
            logger_1.logger.info('Document share updated', {
                shareId,
                permission,
                expiresAt
            });
            ...
            logger_1.logger.error('Error updating document share', { error, shareId, ownerId });
            throw error;
        }
    }
    /**
     ...
     */
    async removeShare(shareId, ownerId) {
        try {
            // Verify the share belongs to the owner
            // @ts-ignore: TypeScript not recognizing documentSharing property
            const share = await this.prisma.documentSharing.findFirst({
            ...
            logger_1.logger.info('Document share removed', { shareId });
        }
        catch (error) {
            logger_1.logger.error('Error removing document share', { error, shareId, ownerId });
            throw error;
        }
    }
    /**
     ...
     */
    async hasAccess(documentId, userId) {
        try {
            ...
            logger_1.logger.error('Error checking document access', { error, documentId, userId });
            throw error;
        }
    }
    /**
    ...
    async getPermission(documentId, userId) {
        try {
            ...
            logger_1.logger.error('Error getting document permission', { error, documentId, userId });
            throw error;
        }
    }
}
exports.DocumentSharingService = DocumentSharingService;
