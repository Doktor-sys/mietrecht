...
describe('EmailService Unit Tests', () => {
    ...
    describe('sendVerificationEmail', () => {
        it('should send verification email successfully', async () => {
            ...
            await expect(emailService.sendVerificationEmail(email, token, userData)).resolves.not.toThrow();
        });
    ...
    describe('sendPasswordResetEmail', () => {
        it('should send password reset email successfully', async () => {
            ...
            await expect(emailService.sendPasswordResetEmail(email, token, userData)).resolves.not.toThrow();
        });
    });
    describe('sendWelcomeEmail', () => {
        it('should send welcome email successfully', async () => {
            ...
            await expect(emailService.sendWelcomeEmail(email, userData)).resolves.not.toThrow();
        });
    });
    describe('sendTemplatedEmail', () => {
        it('should send templated email successfully', async () => {
            ...
            await expect(emailService.sendTemplatedEmail(email, templateName, templateData)).resolves.not.toThrow();
        });
        it('should throw error for non-existent template', async () => {
            const email = 'test@example.com';
            const templateName = 'non-existent';
            const templateData = {};
            await expect(emailService.sendTemplatedEmail(email, templateName, templateData)).rejects.toThrow("Template 'non-existent' nicht gefunden");
        });
    });
    describe('sendEmail', () => {
    ...
    describe('template rendering', () => {
        ...
        it('should handle conditional templates', () => {
            ...
            const renderedWithName = emailService.renderTemplate(template, dataWithName);
            expect(renderedWithName).toBe('Hello, Max!');
            const dataWithoutName = {};
            const renderedWithoutName = emailService.renderTemplate(template, dataWithoutName);
    ...
    describe('email masking', () => {
        it('should mask email addresses correctly', () => {
            const maskEmail = emailService.maskEmail.bind(emailService);
    ...
    describe('rate limiting', () => {
        it('should enforce email rate limiting', async () => {
            ...
            await expect(emailService.checkEmailRateLimit(email)).rejects.toThrow('E-Mail-Rate-Limit überschritten');
        ...
        it('should allow emails within rate limit', async () => {
            ...
            const originalRedis = require('../config/redis').redis;
            require('../config/redis').redis = mockRedis;
            await expect(emailService.checkEmailRateLimit(email)).resolves.not.toThrow();
            // Restore original redis
            require('../config/redis').redis = originalRedis;
        });
    });
