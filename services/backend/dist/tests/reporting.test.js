...
describe('ReportingService', () => {
    ...
    beforeEach(() => {
        ...
        emailServiceMock = new EmailService_1.EmailService();
        EmailService_1.EmailService.mockImplementation(() => emailServiceMock);
        // Initialize service
        reportingService = new ReportingService_1.ReportingService();
    });
    describe('generateComprehensiveReport', () => {
        it('should generate a report and send a notification', async () => {
            ...
            await reportingService.deliverReport(mockOrganization, mockReport);
            // Verify EmailService was called
            expect(emailServiceMock.sendReportNotification).toHaveBeenCalledTimes(1);
            expect(emailServiceMock.sendReportNotification).toHaveBeenCalledWith('test@example.com', expect.objectContaining({
    ...
    describe('exportReport', () => {
        it('should export report as PDF', async () => {
            const mockReport = {
                ...
                recommendations: [],
                alerts: []
            }; // Cast to any to avoid mocking full interface
            const pdfBuffer = Buffer.from('PDF Content');
            pdfGenerator_1.PdfGenerator.generateReport.mockResolvedValue(pdfBuffer);
            const result = await reportingService.exportReport(mockReport, 'pdf');
            expect(pdfGenerator_1.PdfGenerator.generateReport).toHaveBeenCalledWith(mockReport);
