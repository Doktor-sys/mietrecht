...
const DocumentSharingService_1 = require("../services/DocumentSharingService");
const DocumentAnnotationService_1 = require("../services/DocumentAnnotationService");
const DocumentWorkflowService_1 = require("../services/DocumentWorkflowService");
const OCRService_1 = __importDefault(require("../services/OCRService"));
const logger_1 = require("../utils/logger");
const errorHandler_1 = require("../middleware/errorHandler");
const prisma = new client_1.PrismaClient();
const documentStorageService = new DocumentStorageService_1.DocumentStorageService(prisma);
const documentSharingService = new DocumentSharingService_1.DocumentSharingService(prisma);
const documentAnnotationService = new DocumentAnnotationService_1.DocumentAnnotationService(prisma);
const documentWorkflowService = new DocumentWorkflowService_1.DocumentWorkflowService(prisma);
class DocumentController {
    ...
    static async uploadDocument(req, res, next) {
        try {
            ...
            if (!documentType || !Object.values(client_1.DocumentType).includes(documentType)) {
                throw new errorHandler_1.ValidationError('Invalid document type');
            }
            const result = await documentStorageService.uploadDocument(userId, req.file, documentType, caseId);
    ...
    static async searchDocuments(req, res, next) {
        try {
            ...
            const { query, documentType, startDate, endDate, minRiskLevel, maxRiskLevel, sortBy = 'uploadedAt', sortOrder = 'desc', page = 1, limit = 20 } = req.query;
     ...
     */
    static async uploadDocumentVersion(req, res, next) {
        try {
            const userId = req.user?.id;
            if (!userId) {
                throw new errorHandler_1.ValidationError('User not authenticated');
            ...
            if (!documentType) {
                throw new errorHandler_1.ValidationError('Document type is required');
            }
            ...
            if (!parentDocument) {
                throw new errorHandler_1.ValidationError('Parent document not found or access denied');
            }
            const result = await documentStorageService.uploadDocument(userId, file, documentType, undefined, // caseId
            documentId // parentId
            );
            res.json({
                success: true,
    ...
    static async downloadDocument(req, res, next) {
        try {
            ...
            const { documentId } = req.params;
            const { stream, filename, mimeType } = await documentStorageService.downloadDocument(documentId, userId);
    ...
    static async deleteDocument(req, res, next) {
        try {
            ...
            logger_1.logger.info('Document deleted', { userId, documentId });
            res.json({
                success: true,
     ...
     */
    static async extractText(req, res, next) {
        try {
            const userId = req.user?.id;
            if (!userId) {
                throw new errorHandler_1.ValidationError('User not authenticated');
            ...
            const { stream, mimeType } = await documentStorageService.downloadDocument(documentId, userId);
            ...
            if (mimeType === 'application/pdf') {
                ocrResult = await OCRService_1.default.extractTextFromPDF(buffer);
            }
            else if (mimeType.startsWith('image/')) {
                ocrResult = await OCRService_1.default.extractTextFromImage(buffer);
            }
            else {
                throw new errorHandler_1.ValidationError('Unsupported document type for OCR');
            }
            logger_1.logger.info('Text extracted from document', {
                userId,
                documentId,
     ...
     */
    static async analyzeRentalContract(req, res, next) {
        try {
            const userId = req.user?.id;
            if (!userId) {
                throw new errorHandler_1.ValidationError('User not authenticated');
            ...
            if (mimeType === 'application/pdf') {
                ...
                throw new errorHandler_1.ValidationError('Unsupported document type');
            }
            // Preprocess text
            const preprocessed = OCRService_1.default.preprocessGermanLegalText(ocrResult.text);
            // Extract structured data
            const extractedData = OCRService_1.default.extractRentalContractData(ocrResult.text);
            logger_1.logger.info('Rental contract analyzed', {
                userId,
                documentId,
     ...
     */
    static async analyzeUtilityBill(req, res, next) {
        try {
            const userId = req.user?.id;
            if (!userId) {
                throw new errorHandler_1.ValidationError('User not authenticated');
            ...
            const extractedData = OCRService_1.default.extractUtilityBillData(ocrResult.text);
            logger_1.logger.info('Utility bill analyzed', {
                userId,
                documentId,
                fieldsExtracted: Object.keys(extractedData).length
            });
     ...
     */
    static async analyzeWarningLetter(req, res, next) {
        try {
            const userId = req.user?.id;
            if (!userId) {
                throw new errorHandler_1.ValidationError('User not authenticated');
            ...
            const extractedData = OCRService_1.default.extractWarningLetterData(ocrResult.text);
            logger_1.logger.info('Warning letter analyzed', {
                userId,
                documentId,
    ...
    static async shareDocument(req, res, next) {
        try {
            ...
            if (!sharedWithEmail) {
                throw new errorHandler_1.ValidationError('Shared user email is required');
            }
            const share = await documentSharingService.shareDocument(documentId, userId, sharedWithEmail, permission || 'READ', expiresAt ? new Date(expiresAt) : undefined);
            res.json({
                success: true,
                data: share
            });
     ...
     * Get document shares
     */
    static async getDocumentShares(req, res, next) {
        try {
            const userId = req.user?.id;
            if (!userId) {
                throw new errorHandler_1.ValidationError('User not authenticated');
     ...
     * Update a share
     */
    static async updateShare(req, res, next) {
        try {
            const userId = req.user?.id;
            if (!userId) {
                throw new errorHandler_1.ValidationError('User not authenticated');
            ...
            const share = await documentSharingService.updateShare(shareId, userId, permission, expiresAt ? new Date(expiresAt) : undefined);
            res.json({
                success: true,
                data: share
            });
    ...
    static async createAnnotation(req, res, next) {
        try {
            ...
            if (!text) {
                throw new errorHandler_1.ValidationError('Annotation text is required');
            }
            const annotation = await documentAnnotationService.createAnnotation(documentId, userId, text, type, parentId, page, positionX, positionY);
            res.json({
                success: true,
                data: annotation
            });
     ...
     */
    static async getDocumentAnnotations(req, res, next) {
        try {
            const userId = req.user?.id;
            if (!userId) {
                throw new errorHandler_1.ValidationError('User not authenticated');
            ...
            const annotations = await documentAnnotationService.getDocumentAnnotations(documentId, includeReplies !== 'false');
            res.json({
                success: true,
                data: annotations
            });
    ...
    static async updateAnnotation(req, res, next) {
        try {
            ...
            const annotation = await documentAnnotationService.updateAnnotation(annotationId, userId, text, resolved);
            res.json({
                success: true,
                data: annotation
            });
     ...
     */
    static async resolveAnnotation(req, res, next) {
        try {
            const userId = req.user?.id;
            if (!userId) {
                throw new errorHandler_1.ValidationError('User not authenticated');
     ...
     */
    static async transitionDocumentStatus(req, res, next) {
        try {
            const userId = req.user?.id;
            if (!userId) {
                throw new errorHandler_1.ValidationError('User not authenticated');
            ...
            const updatedDocument = await documentWorkflowService.transitionDocumentStatus(documentId, userId, action, comment);
            res.json({
                success: true,
     ...
     */
    static async getDocumentWorkflowHistory(req, res, next) {
        try {
            const userId = req.user?.id;
            if (!userId) {
                throw new errorHandler_1.ValidationError('User not authenticated');
     ...
     */
    static async getWorkflowRules(req, res, next) {
        try {
     ...
     */
    static async createWorkflowRule(req, res, next) {
        try {
            const { name, triggerEvent, action, condition, actionParams, description } = req.body;
            if (!name || !triggerEvent || !action) {
                throw new errorHandler_1.ValidationError('Name, triggerEvent, and action are required');
            }
            const rule = await documentWorkflowService.createWorkflowRule(name, triggerEvent, action, condition, actionParams, description);
            res.status(201).json({
                success: true,
                data: rule
            });
     ...
     */
    static async updateWorkflowRule(req, res, next) {
        try {
     ...
     */
    static async deleteWorkflowRule(req, res, next) {
        try {
            const { ruleId } = req.params;
