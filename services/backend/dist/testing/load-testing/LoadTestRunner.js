...
class LoadTestRunner {
    ...
    async runLoadTest(config) {
        logger_1.logger.info(`Starting load test: ${config.targetUrl}`, {
        ...
        try {
            ...
            const requestsPerWorker = Math.ceil((config.requestsPerSecond * config.duration) / config.concurrency);
            // Erstelle Worker-Promises
            const workerPromises = [];
            for (let i = 0; i < config.concurrency; i++) {
                workerPromises.push(this.runWorker(config, requestsPerWorker, metrics));
            }
            ...
            logger_1.logger.info('Load test completed', result);
            return result;
        }
        catch (error) {
     ...
     */
    async runWorker(config, requestsToSend, metrics) {
        ...
        for (let i = 0; i < requestsToSend && this.isRunning; i++) {
            try {
                ...
                logger_1.logger.warn(`Request failed during load test`, {
                    url: config.targetUrl,
     ...
     */
    async runStressTest(baseUrl, maxConcurrency, stepSize = 10) {
        const results = [];
        logger_1.logger.info(`Starting stress test: ${baseUrl}`, {
            maxConcurrency,
            stepSize
        });
        for (let concurrency = stepSize; concurrency <= maxConcurrency; concurrency += stepSize) {
            ...
            try {
                ...
                if (result.errorRate > 0.1) { // Mehr als 10% Fehler
                    logger_1.logger.warn(`High error rate detected, stopping stress test`, {
                        concurrency,
                ...
                logger_1.logger.error(`Stress test failed at concurrency ${concurrency}`, { error });
                break;
            }
        }
        return results;
     ...
     */
    async runSpikeTest(baseUrl, baselineConcurrency, spikeConcurrency, spikeDuration = 60 // 60 Sekunden
    ) {
        const results = [];
        logger_1.logger.info(`Starting spike test: ${baseUrl}`, {
            baselineConcurrency,
            spikeConcurrency,
            spikeDuration
        });
