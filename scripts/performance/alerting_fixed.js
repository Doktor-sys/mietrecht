...
class PerformanceAlerting {
  ...
  constructor(config = {}) {
    this.config = {
      ...
      enableEmailAlerts: false, // Deaktiviert, da emailNotifier fehlt
  ...
  async checkAndAlert(metrics) {
    ...
    if (metrics.cache && metrics.cache.hitRate !== undefined && metrics.cache.hitRate < this.config.cacheHitRateThreshold) {
      const alert = {
    ...
    if (metrics.database && metrics.database.avgQueryTime !== undefined && metrics.database.avgQueryTime > this.config.databaseQueryTimeThreshold) {
      const alert = {
    ...
    if (metrics.api && metrics.api.avgResponseTime !== undefined && metrics.api.avgResponseTime > this.config.apiResponseTimeThreshold) {
      const alert = {
    ...
    // Check error rate
    if (metrics.api && metrics.api.errorRate !== undefined && metrics.api.errorRate > this.config.errorRateThreshold) {
      const alert = {
        type: 'api_errors',
        severity: 'error',
  ...
  async processAlert(alert) {
    ...
    // Send email notification (deaktiviert, da emailNotifier fehlt)
    if (this.config.enableEmailAlerts) {
      console.log('Email alert would be sent, but email notifications are disabled due to missing emailNotifier module');
    }
