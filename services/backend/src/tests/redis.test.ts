...
describe('Redis Configuration Tests', () => {
  ...
  describe('Session Management', () => {
    ...
    it('should delete sessions', async () => {
      ...
      expect(await redis.getSession(sessionId)).toEqual(sessionData)

      await redis.deleteSession(sessionId)
      expect(await redis.getSession(sessionId)).toBeNull()
    })
  })
  ...
  describe('Rate Limiting', () => {
    it('should increment rate limit counters', async () => {
      ...
      const count1 = await redis.incrementRateLimit(key, windowSeconds)
      expect(count1).toBe(1)

      const count2 = await redis.incrementRateLimit(key, windowSeconds)
      expect(count2).toBe(2)

      const count3 = await redis.incrementRateLimit(key, windowSeconds)
  ...
  describe('Cache Patterns', () => {
    it('should implement getOrSet pattern', async () => {
      ...
      const result1 = await redis.getOrSet(key, fetchFunction, 60)
      expect(fetchCallCount).toBe(1)
      expect(result1.result).toBe('expensive data')

      // Zweiter Aufruf sollte aus Cache kommen
      const result2 = await redis.getOrSet(key, fetchFunction, 60)
    ...
    it('should handle cache misses in getOrSet', async () => {
      ...
      const result = await redis.getOrSet(key, fetchFunction, 60)
  ...
  describe('Error Handling', () => {
    it('should handle Redis connection errors gracefully', async () => {
      ...
      await expect(redis.set(invalidKey, 'value')).rejects.toThrow()
    })
