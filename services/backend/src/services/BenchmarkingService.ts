...
export interface BenchmarkComparison {
  ...
  industryMedian: number;
  percentile: number;
  deviationFromAverage: number; // in Prozent
...
export class BenchmarkingService {
   ...
   */
  async performBenchmarking(query: BenchmarkQuery): Promise<BenchmarkReport> {
    try {
      ...
      logger.error('Error performing benchmarking:', error);
   ...
   */
  private async getIndustryBenchmarks(query: {
    categories?: string[];
    metrics?: string[];
    jurisdiction?: string;
  }): Promise<Record<string, { average: number; median: number; min: number; max: number }>> {
    try {
      ...
      const result: Record<string, { average: number; median: number; min: number; max: number }> = {};
   ...
   */
  private getDefaultIndustryBenchmarks(): Record<string, { average: number; median: number; min: number; max: number }> {
    return {
  ...
  private compareMetrics(
    ...
    industryBenchmarks: Record<string, { average: number; median: number; min: number; max: number }>
  ): BenchmarkComparison[] {
    ...
    for (const [metric, orgValue] of Object.entries(orgMetrics)) {
      ...
      let performanceRating: 'below' | 'average' | 'above' = 'average';
      ...
      if (industry.max > industry.min) {
        percentile = Math.min(100, Math.max(0, 
          ((orgValue - industry.min) / (industry.max - industry.min)) * 100
        ));
      }
  ...
  private generateBenchmarkRecommendations(
    metric: string,
    userValue: number,
    industry: { average: number; median: number; min: number; max: number },
    performanceRating: 'below' | 'average' | 'above'
  ): string[] {
   ...
   */
  async upsertBenchmark(benchmarkData: Omit<BenchmarkData, 'id' | 'createdAt' | 'updatedAt'>): Promise<BenchmarkData> {
    try {
      ...
      logger.error('Error upserting benchmark:', error);
  ...
  async deleteBenchmark(id: string): Promise<void> {
    try {
      ...
      logger.error(`Error deleting benchmark ${id}:`, error);
