...
describe('ChatService', () => {
  ...
  let mockAIResponseGenerator: jest.Mocked<AIResponseGenerator>;
  let mockClassifier: jest.Mocked<LegalCaseClassifier>;

  beforeEach(() => {
    mockWebSocketService = new WebSocketService() as jest.Mocked<WebSocketService>;
    mockAIResponseGenerator = new AIResponseGenerator() as jest.Mocked<AIResponseGenerator>;
    mockClassifier = new LegalCaseClassifier() as jest.Mocked<LegalCaseClassifier>;
  ...
  describe('startConversation', () => {
    ...
    it('sollte Fehler bei ungültigem userId werfen', async () => {
      await expect(
        chatService.startConversation('', 'Test query')
      ).rejects.toThrow('User ID ist erforderlich');
    });
  });
  ...
  describe('sendMessage', () => {
    it('sollte eine Nachricht senden und KI-Antwort generieren', async () => {
      ...
      expect(mockAIResponseGenerator.generateResponse).toHaveBeenCalledWith(
        message,
        expect.any(Object)
      );
