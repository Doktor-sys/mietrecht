...
describe('Email Authentication API Tests', () => {
    ...
    beforeEach(async () => {
        ...
        await redis_1.redis.setSession(sessionId, { userId: testUserId });
        accessToken = jsonwebtoken_1.default.sign({ userId: testUserId, email: user.email, userType: client_1.UserType.TENANT, sessionId, type: 'access' }, config_1.config.jwt.secret, { expiresIn: '1h' });
    });
    describe('POST /api/auth/verify-email', () => {
        it('should verify email with valid token', async () => {
            // Erstelle Verifizierungs-Token
            const verificationToken = jsonwebtoken_1.default.sign({ userId: testUserId, email: 'test@example.com', type: 'email_verification' }, config_1.config.jwt.secret, { expiresIn: '24h' });
            // Speichere Token in Redis
            await redis_1.redis.set(`email_verification:${testUserId}`, verificationToken, 24 * 60 * 60);
        ...
        it('should reject expired verification token', async () => {
            const expiredToken = jsonwebtoken_1.default.sign({ userId: testUserId, type: 'email_verification' }, config_1.config.jwt.secret, { expiresIn: '-1h' });
            const response = await (0, supertest_1.default)(index_1.default)
                .post('/api/auth/verify-email')
        ...
        it('should reject token not in Redis', async () => {
            const validToken = jsonwebtoken_1.default.sign({ userId: testUserId, type: 'email_verification' }, config_1.config.jwt.secret, { expiresIn: '24h' });
        ...
        it('should reject verification for already verified user', async () => {
            ...
            const verificationToken = jsonwebtoken_1.default.sign({ userId: testUserId, type: 'email_verification' }, config_1.config.jwt.secret, { expiresIn: '24h' });
            await redis_1.redis.set(`email_verification:${testUserId}`, verificationToken, 24 * 60 * 60);
            const response = await (0, supertest_1.default)(index_1.default)
                .post('/api/auth/verify-email')
                .send({ token: verificationToken })
    ...
    describe('POST /api/auth/resend-verification', () => {
        ...
        it('should enforce rate limiting', async () => {
            ...
            await redis_1.redis.set(`resend_verification:${testUserId}`, '3', 3600);
            const response = await (0, supertest_1.default)(index_1.default)
                .post('/api/auth/resend-verification')
                .set('Authorization', `Bearer ${accessToken}`)
                .expect(400);
    ...
    describe('POST /api/auth/confirm-email-change', () => {
        it('should confirm email change with valid token', async () => {
            ...
            const changeToken = jsonwebtoken_1.default.sign({
                userId: testUserId,
            ...
            // Speichere Token in Redis
            await redis_1.redis.set(`email_change:${testUserId}`, changeToken, 3600);
            const response = await (0, supertest_1.default)(index_1.default)
        ...
        it('should reject expired change token', async () => {
            const expiredToken = jsonwebtoken_1.default.sign({ userId: testUserId, type: 'email_change' }, config_1.config.jwt.secret, { expiresIn: '-1h' });
            const response = await (0, supertest_1.default)(index_1.default)
                .post('/api/auth/confirm-email-change')
                .send({ token: expiredToken })
                .expect(401);
        ...
        it('should reject token not in Redis', async () => {
            const validToken = jsonwebtoken_1.default.sign({ userId: testUserId, type: 'email_change', newEmail: 'new@example.com' }, config_1.config.jwt.secret, { expiresIn: '1h' });
            // Token nicht in Redis speichern
            const response = await (0, supertest_1.default)(index_1.default)
                .post('/api/auth/confirm-email-change')
                .send({ token: validToken })
