...
const metricsStore: {
  apiCalls: Map<string, { count: number; totalDuration: number; errors: number }>;
  mlProcessing: Map<string, { count: number; totalDuration: number; errors: number }>;
  system: {
...
class MonitoringController {
   ...
   */
  static recordMLProcessing(functionName: string, duration: number, success: boolean) {
  ...
  static getMetrics() {
    ...
    const mlMetrics: Record<string, any> = {};
   ...
   */
  static generateAlerts(metrics: any) {
    const alerts = [];

    // Check for high error rates in API calls
    for (const [endpoint, stats] of Object.entries(metrics.apiMetrics) as [string, any]) {
    ...
    for (const [functionName, stats] of Object.entries(metrics.mlMetrics) as [string, any]) {
   ...
   * Reset all metrics
   */
  static resetMetrics(req: Request, res: Response, next: NextFunction) {
    try {
