@echo off
::
:: SmartLaw Mietrecht - Complete Local Testing Environment 🧪
:: ========================================================
::
:: Umgebungsvariablen:
...
::                    Setze auf 1 um die Prüfung zu überschreiben
...
echo ========================================================
echo   SmartLaw Mietrecht - Complete Local Testing Environment 🧪
echo ========================================================
echo.
...
set "LOGFILE=%~dp0start_local_complete.log"
...
if exist "%LOGFILE%" (
        ...
        if defined LOGSIZE if %LOGSIZE% GTR 5242880 (
                ...
                for /f "delims=" %%F in ('cmd /c "dir /b /o:-d "%CD%\start_local_complete.log.*" 2>nul"') do (
...
echo [1/4] Syncing Database...
echo [1/4] Syncing Database... >> "%LOGFILE%"
...
echo [2/4] Starting Backend... >> "%LOGFILE%"
set "PORT=3001"
...
REM Warte kurz auf Backend-Start
echo [3/4] Waiting for Backend to start...
echo [3/4] Waiting for Backend to start... >> "%LOGFILE%"
timeout /t 10 /nobreak >nul

REM Starte die Web App
echo [4/4] Starting Web App...
echo [4/4] Starting Web App... >> "%LOGFILE%"
pushd web-app
if not exist "node_modules\react-scripts\package.json" (
        ...
        echo [Info] Installing/updating dependencies with legacy peer deps...
        echo [Info] Installing/updating dependencies with legacy peer deps... >> "%LOGFILE%"
        call npm install --legacy-peer-deps >> "%LOGFILE%" 2>&1
)
