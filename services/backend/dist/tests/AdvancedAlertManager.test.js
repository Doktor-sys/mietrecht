...
const AdvancedAlertManager_1 = require("../services/AdvancedAlertManager");
...
describe('AdvancedAlertManager', () => {
    ...
    beforeEach(() => {
        ...
        alertManager = new AdvancedAlertManager_1.AdvancedAlertManager(config);
    });
    describe('Alert Creation', () => {
        it('should create an alert with correct properties', () => {
            const alert = alertManager.createAlert(AlertManager_1.AlertSeverity.CRITICAL, 'Test Alert', 'This is a test alert', { test: 'data' });
            expect(alert).toBeDefined();
            expect(alert.severity).toBe(AlertManager_1.AlertSeverity.CRITICAL);
        ...
        it('should store alerts internally', () => {
            const alert = alertManager.createAlert(AlertManager_1.AlertSeverity.WARNING, 'Storage Test', 'Testing alert storage');
    ...
    describe('Alert Resolution', () => {
        it('should resolve an existing alert', () => {
            const alert = alertManager.createAlert(AlertManager_1.AlertSeverity.ERROR, 'Resolution Test', 'Testing alert resolution');
    ...
    describe('Alert Deduplication', () => {
        it('should prevent duplicate alerts within deduplication window', () => {
            // Create first alert
            const alert1 = alertManager.createAlert(AlertManager_1.AlertSeverity.CRITICAL, 'Duplicate Test', 'This is a duplicate test message');
            // Create duplicate alert immediately
            const alert2 = alertManager.createAlert(AlertManager_1.AlertSeverity.CRITICAL, 'Duplicate Test', 'This is a duplicate test message');
        ...
        it('should allow alerts after deduplication window expires', (done) => {
            // Create first alert
            alertManager.createAlert(AlertManager_1.AlertSeverity.WARNING, 'Timing Test', 'This is a timing test message');
            // Wait for deduplication window to expire
            setTimeout(() => {
                const alert2 = alertManager.createAlert(AlertManager_1.AlertSeverity.WARNING, 'Timing Test', 'This is a timing test message');
    ...
    describe('Alert Statistics', () => {
        it('should provide correct alert statistics', () => {
            // Create some alerts
            alertManager.createAlert(AlertManager_1.AlertSeverity.CRITICAL, 'Critical 1', 'Message 1');
            alertManager.createAlert(AlertManager_1.AlertSeverity.ERROR, 'Error 1', 'Message 2');
            alertManager.createAlert(AlertManager_1.AlertSeverity.WARNING, 'Warning 1', 'Message 3');
            alertManager.createAlert(AlertManager_1.AlertSeverity.INFO, 'Info 1', 'Message 4');
            const stats = alertManager.getStatistics();
            expect(stats.total).toBe(4);
            expect(stats.active).toBe(4);
            expect(stats.resolved).toBe(0);
            expect(stats.bySeverity[AlertManager_1.AlertSeverity.CRITICAL]).toBe(1);
            expect(stats.bySeverity[AlertManager_1.AlertSeverity.ERROR]).toBe(1);
            expect(stats.bySeverity[AlertManager_1.AlertSeverity.WARNING]).toBe(1);
            expect(stats.bySeverity[AlertManager_1.AlertSeverity.INFO]).toBe(1);
        });
        it('should update statistics when alerts are resolved', () => {
            const alert = alertManager.createAlert(AlertManager_1.AlertSeverity.CRITICAL, 'Resolution Stats Test', 'Testing stats with resolution');
    ...
    describe('Service Integration', () => {
        ...
        it('should register handlers for all severity levels', () => {
            Object.values(AlertManager_1.AlertSeverity).forEach(severity => {
                expect(alertManager['alertHandlers'].has(severity)).toBe(true);
            });
        });
    });
});
