...
exports.detectSuspiciousUserAgents = detectSuspiciousUserAgents;
...
function detectSuspiciousUserAgents(req, res, next) {
    ...
    for (const blockedAgent of SECURITY_CONFIG.blockedUserAgents) {
        if (userAgent.toLowerCase().includes(blockedAgent)) {
            logger_1.logger.warn(`Blocked user agent detected: ${userAgent}`, {
                ip,
                url: req.url
            });
            // Blockiere sofort
...
function checkIPReputation(req, res, next) {
    ...
    if (metrics.suspiciousActivities > 3) {
        logger_1.logger.warn(`IP with bad reputation blocked: ${ip}`, {
    ...
    if (metrics.blockedRequests > 10) {
        logger_1.logger.warn(`IP with excessive blocked requests: ${ip}`, {
...
function contentSecurity(req, res, next) {
    ...
    if (req.path.startsWith('/api/kms') || req.path.startsWith('/api/admin')) {
        if (!contentType.includes('application/json')) {
            logger_1.logger.warn('Invalid content type for sensitive endpoint', {
                ip,
                path: req.path,
                contentType
            });
