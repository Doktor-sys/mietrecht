...
export class TrendAnalysisService {
   ...
   */
  async analyzeTrends(query: TrendQuery = {}): Promise<LegalTrendAnalysis> {
    try {
   ...
   */
  private filterTrends(trends: LegalTrend[], query: TrendQuery): LegalTrend[] {
    ...
    let filtered = trends.filter(trend => {
      ...
      if (trend.relevanceScore < minRelevance) return false;

      // Filter nach Startdatum
      if (startDate && trend.startDate < startDate) return false;

      // Filter nach Enddatum
      if (endDate && trend.startDate > endDate) return false;

      // Filter nach Kategorie
      if (category && trend.category !== category) return false;

      // Filter nach Jurisdiktion
      if (jurisdiction && trend.jurisdiction !== jurisdiction) return false;
   ...
   */
  private generateTrendAnalysis(trends: LegalTrend[], query: TrendQuery): LegalTrendAnalysis {
    // Zeitraum bestimmen
    const period = {
    ...
    const trendTypes: Record<string, number> = {};
    trends.forEach(trend => {
   ...
   */
  private generateTrendRecommendations(trends: LegalTrend[], query: TrendQuery): string[] {
  ...
  async createTrend(trendData: Omit<LegalTrend, 'id' | 'createdAt' | 'updatedAt'>): Promise<LegalTrend> {
    try {
      ...
      logger.error('Error creating legal trend:', error);
   ...
   */
  async updateTrend(id: string, trendData: Partial<LegalTrend>): Promise<LegalTrend> {
    try {
      ...
      logger.error(`Error updating legal trend ${id}:`, error);
  ...
  async deleteTrend(id: string): Promise<void> {
    try {
      ...
      logger.error(`Error deleting legal trend ${id}:`, error);
   ...
   */
  async findSimilarTrends(trendId: string, limit: number = 5): Promise<LegalTrend[]> {
    try {
      ...
      logger.error(`Error finding similar trends to ${trendId}:`, error);
