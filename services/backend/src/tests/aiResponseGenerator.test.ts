...
describe('AIResponseGenerator', () => {
  ...
  describe('generateResponse', () => {
    it('should generate response with legal references for rent reduction', async () => {
      ...
      const response = await generator.generateResponse(
        classification,
        'Meine Heizung ist seit 2 Wochen kaputt'
      );
    ...
    it('should include action recommendations', async () => {
      ...
      const docAction = response.actionRecommendations.find(
        action => action.action.includes('dokumentieren')
      );
    ...
    it('should include template references for rent reduction', async () => {
      ...
      const template = response.templateReferences.find(
        t => t.templateId === 'rent_reduction_notice'
      );
    ...
    it('should handle utility costs category', async () => {
      const classification: ClassificationResult = {
        classification: {
          category: 'utility_costs' as LegalCategory,
          confidence: 0.8,
          riskLevel: 'low',
          escalationRecommended: false,
      ...
      const template = response.templateReferences.find(
        t => t.templateId === 'utility_objection'
      );
      expect(template).toBeDefined();
    });
    ...
    it('should add urgent action for high urgency cases', async () => {
      ...
      const urgentAction = response.actionRecommendations.find(
        action => action.priority === 'high' && action.deadline === 'Dringend'
      );
    ...
    it('should handle rent increase category', async () => {
      const classification: ClassificationResult = {
        classification: {
          category: 'rent_increase' as LegalCategory,
          confidence: 0.85,
          riskLevel: 'medium',
          escalationRecommended: false,
          estimatedComplexity: 'moderate'
      ...
      const deadlineAction = response.actionRecommendations.find(
        action => action.deadline === '2 Monate'
      );
    ...
    it('should handle deposit category', async () => {
      ...
      const template = response.templateReferences.find(
        t => t.templateId === 'deposit_return_request'
      );
      expect(template).toBeDefined();
    });
  ...
  describe('Action Recommendations', () => {
    it('should prioritize actions correctly', async () => {
      ...
      const highPriorityActions = response.actionRecommendations.filter(
    ...
    it('should include deadlines where applicable', async () => {
      ...
      const actionsWithDeadlines = response.actionRecommendations.filter(
