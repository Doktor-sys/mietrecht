 ...
 */
router.get('/notifications', authenticate, authorize(UserType.BUSINESS), async (req: Request, res: Response) => {
  try {
 ...
 *         description: Forbidden
 */
router.get('/chat', authenticate, authorize(UserType.BUSINESS), async (req: Request, res: Response) => {
  try {
    // In a real implementation, this would fetch data from a database or cache
    const stats = {
      totalMessages: 2847,
 ...
 *         description: Forbidden
 */
router.get('/stream', authenticate, authorize(UserType.BUSINESS), (req: Request, res: Response) => {
  // Set SSE headers
  res.writeHead(200, {
