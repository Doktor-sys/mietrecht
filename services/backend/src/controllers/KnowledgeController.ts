...
import { KnowledgeService, SearchFilters } from '../services/KnowledgeService'
import { prisma } from '../config/database'
import { asyncHandler } from '../middleware/errorHandler'
import { ValidationError } from '../middleware/errorHandler'
import { LegalType, UserType } from '@prisma/client'
 ...
 */
  getLegalText = asyncHandler(async (req: Request, res: Response) => {
    const { reference } = req.params

    if (!reference) {
      throw new ValidationError('Referenz ist erforderlich')
    }

    const legalText = await this.knowledgeService.getLegalText(reference)

    if (!legalText) {
      throw new ValidationError('Rechtstext nicht gefunden')
    }
   ...
   *         $ref: '#/components/responses/NotFoundError'
   */
  findSimilarContent = asyncHandler(async (req: Request, res: Response) => {
    const { reference } = req.params
  ...
  updateKnowledgeBase = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (userType !== UserType.BUSINESS) {
      throw new ValidationError('Nicht autorisiert für Datenbank-Updates')
    }
   ...
   *         $ref: '#/components/responses/AuthorizationError'
   */
  addLegalContent = asyncHandler(async (req: Request, res: Response) => {
    const userType = req.user?.userType

    if (userType !== UserType.BUSINESS) {
      throw new ValidationError('Nicht autorisiert für das Hinzufügen von Rechtsinhalten')
    }
   ...
   *         $ref: '#/components/responses/NotFoundError'
   */
  updateLegalContent = asyncHandler(async (req: Request, res: Response) => {
    const userType = req.user?.userType
    const { reference } = req.params

    if (userType !== UserType.BUSINESS) {
      throw new ValidationError('Nicht autorisiert für das Aktualisieren von Rechtsinhalten')
    }
    ...
    if (req.body.jurisdiction) updateData.jurisdiction = req.body.jurisdiction
   ...
   *         $ref: '#/components/responses/NotFoundError'
   */
  deleteLegalContent = asyncHandler(async (req: Request, res: Response) => {
    const userType = req.user?.userType
    const { reference } = req.params

    if (userType !== UserType.BUSINESS) {
      throw new ValidationError('Nicht autorisiert für das Löschen von Rechtsinhalten')
    }
   ...
   *         $ref: '#/components/responses/AuthorizationError'
   */
  initializeIndex = asyncHandler(async (req: Request, res: Response) => {
    const userType = req.user?.userType

    if (userType !== UserType.BUSINESS) {
      throw new ValidationError('Nicht autorisiert für Index-Initialisierung')
    }
