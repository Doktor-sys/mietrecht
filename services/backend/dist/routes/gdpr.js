...
const gdprService = new GDPRComplianceService_1.GDPRComplianceService(prisma);
/**
 * @swagger
 * /api/gdpr/export:
 *   post:
...
router.post('/export', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        logger_1.logger.error('GDPR export failed:', error);
        res.status(500).json({
            success: false,
 ...
 *         description: Unauthorized
 */
router.post('/delete', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user.userId;
        ...
        logger_1.logger.error('GDPR deletion failed:', error);
        res.status(500).json({
            success: false,
 ...
 *         description: Unauthorized
 */
router.get('/consent', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user.userId;
 ...
 *         description: Unauthorized
 */
router.put('/consent', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user.userId;
