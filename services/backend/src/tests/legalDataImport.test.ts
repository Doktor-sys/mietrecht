import { LegalDataImportService, LegalDataImport } from '../services/LegalDataImportService'
...
describe('LegalDataImportService Unit Tests', () => {
  let service: LegalDataImportService
  let prismaMock: any
  ...
  beforeEach(() => {
    ...
    service = new LegalDataImportService(prismaMock)
  })

  describe('Validierung', () => {
    it('sollte gültige Import-Daten akzeptieren', async () => {
      const validData: LegalDataImport[] = [
        {
          type: 'LAW' as LegalType,
          reference: '§ 536 BGB',
          title: 'Mietminderung',
          content: 'Inhalt...',
    ...
    it('sollte Fehler für fehlende Referenz werfen', async () => {
      const invalidData: LegalDataImport[] = [
        {
          type: 'LAW' as LegalType,
          reference: '',
          title: 'Test',
  ...
  describe('BGB-Import', () => {
    it('sollte BGB-Paragraphen korrekt importieren', async () => {
      ...
      expect(result.success).toBe(true)
      expect(result.imported).toBe(1)
      expect(prismaMock.legalKnowledge.create).toHaveBeenCalledWith(
  ...
  describe('Update-Funktionalität', () => {
    ...
    it('sollte Fehler werfen wenn Daten nicht gefunden werden', async () => {
      ...
      await expect(
        service.updateLegalData('§ 999 BGB', { title: 'Test' })
  ...
  describe('Batch-Verarbeitung', () => {
    it('sollte große Datenmengen in Batches verarbeiten', async () => {
      const largeDataset: LegalDataImport[] = Array.from({ length: 250 }, (_, i) => ({
        type: 'LAW' as LegalType,
        reference: `§ ${i + 1} BGB`,
        title: `Paragraph ${i + 1}`,
        content: 'Inhalt...',
