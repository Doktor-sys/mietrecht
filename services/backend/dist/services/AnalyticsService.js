...
const alertManager = new AlertManager_1.AlertManager(prisma);
class AnalyticsService {
    constructor() {
        ...
        this.complianceMonitoringService = new ComplianceMonitoringService_1.ComplianceMonitoringService(prisma, alertManager);
    }
    /**
     * Generiert umfassende Analytics für eine Organisation
     */
    async generateAnalytics(query) {
        try {
            const { organizationId, startDate, endDate, groupBy } = query;
            // Parallel alle Metriken sammeln
            const [apiRequests, documentAnalyses, chatInteractions, templateGenerations, bulkJobs, apiKey] = await Promise.all([
            ...
            logger_1.logger.error('Error generating analytics:', error);
    ...
    async generateUsageReport(organizationId, period) {
        try {
            ...
            logger_1.logger.error('Error generating usage report:', error);
     ...
     */
    async getApiRequestMetrics(organizationId, startDate, endDate) {
    ...
    async getDocumentAnalysisMetrics(organizationId, startDate, endDate) {
    ...
    async getChatInteractionMetrics(organizationId, startDate, endDate) {
    ...
    async getTemplateGenerationMetrics(organizationId, startDate, endDate) {
    ...
    async getBulkJobMetrics(organizationId, startDate, endDate) {
    ...
    async getQuotaInfo(organizationId) {
    ...
    async generateTrends(organizationId, startDate, endDate, groupBy) {
        ...
        requests.forEach(r => {
            ...
            if (!trendMap[key])
                trendMap[key] = { date: key, requests: 0, documents: 0, chats: 0 };
