...
class PerformanceReportGenerator {
  ...
  generateRecommendations(metrics) {
    ...
    if (metrics.cache && metrics.cache.hitRate !== undefined && metrics.cache.hitRate < 0.7) {
    ...
    if (metrics.database && metrics.database.avgQueryTime !== undefined && metrics.database.avgQueryTime > 100) {
      recommendations.push({
        priority: 'medium',
        message: 'Average database query time exceeds 100ms. Additional indexes have been added for better performance.',
        action: 'Review database indexes and queries'
      });
    }
    
    // Check API performance
    if (metrics.api && metrics.api.avgResponseTime !== undefined && metrics.api.avgResponseTime > 2000) {
      recommendations.push({
        priority: 'high',
  ...
  generateSummary(metrics) {
    // Ensure metrics exist before accessing properties
    const cacheHitRate = metrics.cache && metrics.cache.hitRate !== undefined ? metrics.cache.hitRate : null;
    const avgDatabaseQueryTime = metrics.database && metrics.database.avgQueryTime !== undefined ? metrics.database.avgQueryTime : null;
    const avgApiResponseTime = metrics.api && metrics.api.avgResponseTime !== undefined ? metrics.api.avgResponseTime : null;
    
    return {
      overallHealth: this.calculateOverallHealth(metrics),
      keyMetrics: {
        cacheHitRate: cacheHitRate,
        avgDatabaseQueryTime: avgDatabaseQueryTime,
  ...
  calculateOverallHealth(metrics) {
    ...
    if (metrics.cache && metrics.cache.hitRate !== undefined) {
      if (metrics.cache.hitRate < 0.5) score -= 30;
      else if (metrics.cache.hitRate < 0.7) score -= 15;
    }
    
    // Check database performance
    if (metrics.database && metrics.database.avgQueryTime !== undefined) {
      if (metrics.database.avgQueryTime > 200) score -= 20;
      else if (metrics.database.avgQueryTime > 100) score -= 10;
    }
    
    // Check API performance
    if (metrics.api && metrics.api.avgResponseTime !== undefined) {
      if (metrics.api.avgResponseTime > 5000) score -= 30;
      else if (metrics.api.avgResponseTime > 2000) score -= 15;
    }
...
<body>
    ...
    <div class="metric-card">
        ...
        <p>Hits: ${report.performanceMetrics.cache.hits || 'N/A'}</p>
        <p>Misses: ${report.performanceMetrics.cache.misses || 'N/A'}</p>
        <p>Hit Rate: ${report.performanceMetrics.cache.hitRate !== undefined ? (report.performanceMetrics.cache.hitRate * 100).toFixed(2) + '%' : 'N/A'}</p>
        <p>Current Size: ${report.performanceMetrics.cache.currentSize || 'N/A'}/${report.performanceMetrics.cache.maxSize || 'N/A'}</p>
        ` : '<p>No cache metrics available</p>'}
        
        <h3>Database Performance</h3>
        ${report.performanceMetrics.database ? `
        <p>Total Queries: ${report.performanceMetrics.database.totalQueries || 'N/A'}</p>
        <p>Average Query Time: ${report.performanceMetrics.database.avgQueryTime !== undefined ? report.performanceMetrics.database.avgQueryTime.toFixed(2) + 'ms' : 'N/A'}</p>
        <p>Slowest Query: ${report.performanceMetrics.database.slowestQueryTime !== undefined ? report.performanceMetrics.database.slowestQueryTime.toFixed(2) + 'ms' : 'N/A'}</p>
        ` : '<p>No database metrics available</p>'}
        
        <h3>API Performance</h3>
        ${report.performanceMetrics.api ? `
        <p>Total Requests: ${report.performanceMetrics.api.totalRequests || 'N/A'}</p>
        <p>Average Response Time: ${report.performanceMetrics.api.avgResponseTime !== undefined ? report.performanceMetrics.api.avgResponseTime.toFixed(2) + 'ms' : 'N/A'}</p>
        <p>Error Rate: ${report.performanceMetrics.api.errorRate !== undefined ? (report.performanceMetrics.api.errorRate * 100).toFixed(2) + '%' : 'N/A'}</p>
