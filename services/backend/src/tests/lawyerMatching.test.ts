...
describe('LawyerMatchingService', () => {
  ...
  beforeEach(() => {
    ...
    service = new LawyerMatchingService(prisma);
    jest.clearAllMocks();
  });
  ...
  describe('searchLawyers', () => {
    it('should search lawyers with basic criteria', async () => {
      jest.spyOn(prisma.lawyer, 'findMany').mockResolvedValue(mockLawyers as any);
    ...
    it('should filter by minimum rating', async () => {
      jest.spyOn(prisma.lawyer, 'findMany').mockResolvedValue([mockLawyers[0]] as any);
    ...
    it('should filter by max hourly rate', async () => {
      jest.spyOn(prisma.lawyer, 'findMany').mockResolvedValue([mockLawyers[1]] as any);
      jest.spyOn(prisma.lawyer, 'count').mockResolvedValue(1);
  ...
  describe('findBestMatches', () => {
    it('should find best matches for a case', async () => {
      ...
      const matches = await service.findBestMatches(
        'Meine Heizung ist kaputt und ich möchte die Miete mindern',
        'Berlin, 10115',
        3
      );
    ...
    it('should extract specializations from case description', async () => {
      ...
      const matches = await service.findBestMatches(
        'Der Vermieter will mir kündigen',
        'Berlin, 10115',
        3
      );
  ...
  describe('getLawyerProfile', () => {
    it('should get lawyer profile with details', async () => {
      jest.spyOn(prisma.lawyer, 'findUnique').mockResolvedValue(mockLawyers[0] as any);
  ...
  describe('addReview', () => {
    it('should add review for completed booking', async () => {
      ...
      jest.spyOn(prisma.lawyerReview, 'create').mockResolvedValue({} as any);
      jest.spyOn(prisma.lawyerReview, 'findMany').mockResolvedValue([
        { rating: 5 },
        { rating: 4 }
      ] as any);
      jest.spyOn(prisma.lawyer, 'update').mockResolvedValue({} as any);
    ...
    it('should throw error for invalid rating', async () => {
      ...
      await expect(
        service.addReview('lawyer-1', 'user-1', 'booking-1', 6)
    ...
    it('should throw error for non-completed booking', async () => {
      ...
      await expect(
        service.addReview('lawyer-1', 'user-1', 'booking-1', 5)
    ...
    it('should throw error for already reviewed booking', async () => {
      ...
      jest.spyOn(prisma.booking, 'findUnique').mockResolvedValue(mockBooking as any);
      jest.spyOn(prisma.lawyerReview, 'findUnique').mockResolvedValue(mockReview as any);

      await expect(
        service.addReview('lawyer-1', 'user-1', 'booking-1', 5)
      ).rejects.toThrow('Booking already reviewed');
    });
  });
  ...
  describe('calculateMatchScore', () => {
    it('should calculate high score for perfect match', async () => {
      ...
      const matches = await service.findBestMatches(
        'Ich brauche Hilfe bei Mietminderung und Kündigung',
        'Berlin, 10115',
        1
      );
