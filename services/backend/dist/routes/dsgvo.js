...
const EnhancedDSGVOComplianceService_1 = require("../services/EnhancedDSGVOComplianceService");
...
const dsgvoService = new EnhancedDSGVOComplianceService_1.EnhancedDSGVOComplianceService(prisma, auditService);
/**
 * @swagger
 ...
 */
router.post('/data-subject-requests', auth_1.authenticateToken, async (req, res) => {
    try {
 ...
 */
router.post('/data-subject-requests/:requestId/process', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res) => {
    try {
 ...
 *         description: Nur für Administratoren
 */
router.post('/data-subject-requests/:requestId/reject', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res) => {
    try {
        const { requestId } = req.params;
 ...
 *         description: Nicht autorisiert
 */
router.get('/data-subject-requests', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user.userId;
        ...
        logger_1.logger.error('Failed to get data subject requests:', error);
        res.status(500).json({
            success: false,
 ...
 *         description: Nur für Administratoren
 */
router.get('/data-subject-requests/pending', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res) => {
    try {
 ...
 *         description: Nicht autorisiert
 */
router.post('/consents', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user.userId;
 ...
 *         description: Nicht autorisiert
 */
router.post('/consents/:consentId/withdraw', auth_1.authenticateToken, async (req, res) => {
    try {
 ...
 *         description: Nicht autorisiert
 */
router.get('/consents', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user.userId;
        ...
        logger_1.logger.error('Failed to get consents:', error);
        res.status(500).json({
            success: false,
 ...
 *         description: Nur für Administratoren
 */
router.post('/data-breaches', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res) => {
    try {
 ...
 *         description: Nur für Administratoren
 */
router.get('/compliance-report', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res) => {
    try {
        const { startDate, endDate } = req.query;
        const report = await dsgvoService.generateEnhancedDSGVOComplianceReport(startDate ? new Date(startDate) : new Date(Date.now() - 30 * 24 * 60 * 60 * 1000), endDate ? new Date(endDate) : new Date());
        res.json({
            success: true,
            data: report
        });
